...
class KeyFrameSet(BaseModel):
    ...
    representative_frame: Frame = Field(description="Single frame for shot grouping agent (Step 1)")
...
class SpeakerSegment(BaseModel):
    speaker_id: str = Field(description="Local speaker ID from diarization (e.g., 'SP_A')")
...
class SpeakerInfo(BaseModel):
    ...
    first_seen_scene: str | None = Field(default=None, description="First scene ID where this speaker appears")
    provisional_name: str | None = Field(default=None, description="Provisional name from VLM analysis")
    confidence: str = Field(default="unconfirmed", description="'confirmed' or 'unconfirmed'")
...
class MusicSegment(BaseModel):
    ...
    lyrics_excerpt: str | None = Field(default=None, description="Lyrics excerpt if available")
    ...
    valence: str | None = Field(default=None, description="Valence: 'low', 'medium', 'high'")
    arousal: str | None = Field(default=None, description="Arousal: 'low', 'medium', 'high'")
...
class ShotGroup(BaseModel):
    shots: list[str] = Field(description="Shot IDs belonging to this scene, e.g., ['sh011', 'sh012', 'sh013']")
    grouping_reason: str = Field(description="Reason for grouping: visual/audio/narrative logic")
...
class GroupingBatchResult(BaseModel):
    new_scene_groups: list[ShotGroup] = Field(description="Scene groups identified in this batch")
    ...
    summary_update: str = Field(description="Summary of processed content, for next batch context")
...
class FilmSceneContent(BaseModel):
    visual_description: str = Field(description="Comprehensive visual description of the scene")
    music: str = Field(description="Music description (BGM or insert song)")
    dialogue_and_sounds: str = Field(description="Dialogue and sound events in timestamp order")
    characters_present: list[str] = Field(description="Character names seen in this scene")
    characters_mentioned: list[str] = Field(default_factory=list, description="Characters mentioned but not seen")
...
class FilmSceneIndex(BaseModel):
    metadata: dict[str, Any] = Field(description="Version, generated_at, project, source info")
